...
from apex_config import SymbolConfig, StrategyConfig, get_idm_for_instrument_count

logger = logging.getLogger(__name__)

# H1: Increased risk parameters for larger lot sizes
MAX_MARGIN_FRACTION = 0.75    # Increased from 0.60
MAX_NOTIONAL_FRACTION = 0.95  # Increased from 0.85
MIN_VOLUME_FRACTION = 0.02

MAX_ANNUAL_VOL = 6.0          # Increased from 5.0
MIN_ANNUAL_VOL = 0.80


class ApexVolatilitySizer:

    def __init__(self, strat_cfg: StrategyConfig, vol_sizing_cfg, active_symbols_count: int, symbol_states: dict = None):
        self.strat_cfg = strat_cfg
        self.vol_sizing_cfg = vol_sizing_cfg
        self.expected_positions = max(vol_sizing_cfg.expected_positions, 1)
        self.idm = get_idm_for_instrument_count(active_symbols_count)
        self.symbol_states = symbol_states or {}
        self._usd_zar_rate = None
        self._last_rate_time = 0

        logger.info(f"APEX SIZER (ULTRA - H1) | N={self.expected_positions} | IDM={self.idm:.2f}")
    ...
    def calculate_notional_exposure(self, balance_zar: float, annual_vol: float, forecast: float, symbol: str = None) -> float:
        N = max(self.expected_positions, 1)
        Capital = balance_zar
        T = self.vol_sizing_cfg.risk_target_pct
        V = annual_vol if annual_vol > 0 else 0.01
        F = min(abs(forecast), 50.0)
        IDM = self.idm

        notional_zar = (1.0 / N) * Capital * (T / V) * (F / 8.0) * IDM

        size_multiplier = 1.0
        if symbol and symbol in self.symbol_states:
            state = self.symbol_states[symbol]
            if hasattr(state, 'get_position_size_multiplier'):
                size_multiplier = state.get_position_size_multiplier()
                if size_multiplier < 1.0:
                    notional_zar *= size_multiplier
    ...
    def annual_vol_from_atr(self, atr_daily: float, price_usd: float, symbol: str = "") -> float:
        ...
        annual_vol = min(annual_vol, MAX_ANNUAL_VOL)

        return annual_vol

    def calculate_lots(self, symbol: str, forecast: float, atr_daily: float, sym_cfg: SymbolConfig) -> float:
        ...
        annual_vol = self.annual_vol_from_atr(atr_daily, price_usd, symbol)
        notional_zar = self.calculate_notional_exposure(balance_zar, annual_vol, forecast, symbol)
        ...
        # H1: Allow larger lots for ETH
        if "ETH" in symbol and lots < min_lot and lots > min_lot * 0.5:
            lots = min_lot
        ...
        if total_margin_zar > max_margin_zar and lot_step > 0 and margin_per_lot_zar > 0:
            max_lots_by_margin = math.floor(max_margin_zar / margin_per_lot_zar / lot_step) * lot_step
        ...
        logger.info(
            f"APEX SIZER {symbol}: Bal={balance_zar:.0f} | Fcst={forecast:+.1f} | "
            f"Vol={annual_vol*100:.0f}% | Risk={risk_percent:.0f}% | Lots={lots:.4f}"
        )
