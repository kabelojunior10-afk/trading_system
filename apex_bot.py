...
class ApexTradingBot:
    ...
    def __init__(self, config: SystemConfig):
        ...
        logger.info(f"Timeframe: M1 (1 minute) ultra aggressive")
        logger.info(f"Exit: Trailing stop (4.0× ATR) + Stop and Reverse")
    ...
    def _validate_and_init_symbols(self) -> None:
        ...
        print(f"  ULTRA AGGRESSIVE MODE ACTIVE (M1)")
        print()
    ...
    def run(self) -> None:
        logger.info(" APEX main loop started (ULTRA AGGRESSIVE MODE - M1)")
        while True:
            try:
                t0 = time.time()
    ...
    def _process_symbol(self, symbol: str, state: ApexSymbolState) -> None:
        ...
        state.current_stop_multiplier = 4.0 if state.config.primary_tf == "M1" else result.stop_multiplier
    ...
    def _open_position(self, symbol: str, state: ApexSymbolState, signal: SignalType, result: ForecastResult) -> None:
        ...
        # ── M1 uses wider stops (4x ATR) ──
        initial_stop = None
        stop_distance = 0.0
        if self.cfg.atr_trailing.enabled and result.atr_candle > 0:
            if state.config.primary_tf == "M1":
                stop_mult = 4.0  # Much wider for M1
            else:
        ...
        if ticket:
            ...
            if initial_stop:
                ...
                state.position_state.stop_multiplier = stop_mult if self.cfg.atr_trailing.enabled else 4.0
