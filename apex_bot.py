...
class ApexTradingBot:
    ...
    def __init__(self, config: SystemConfig):
        ...
        for sym, state in self.symbol_states.items():
            self.strategies[sym] = MultiSpeedEWMAC(
                config.strategy,
                timeframe=state.config.primary_tf,
                breakout_weight=config.strategy.breakout_weight,
                ewmac_weight=config.strategy.ewmac_weight,
            )
        ...
        logger.info(f"Timeframe: H1 (1 Hour) aggressive trend following")
        logger.info(f"Strategy: EWMAC + Breakout (60% / 40%)")
        logger.info(f"Exit: Trailing stop (2.2x ATR) + Stop and Reverse")
        logger.info(f"Smart re-entry: ENABLED (30 min cooldown)")

    def _validate_and_init_symbols(self) -> None:
        print("\n" + "═" * 80)
        print(f"  APEX SYSTEM - SYMBOL VALIDATION (H1 TIMEFRAME)")
        print("═" * 80)
        ...
        for sym, sym_cfg in self.cfg.symbols.items():
            ...
            stop_mult = 2.2  # H1 stop - balanced for crypto
            print(
                f"  {sym:<12} {'✓ APEX':<10} "
        ...
        print("═" * 80)
        print(f"  Active symbols: {len(self.symbol_states)}")
        print(f"  AGGRESSIVE H1 MODE ACTIVE")
        print()
    ...
    def run(self) -> None:
        logger.info(f" APEX main loop started (AGGRESSIVE H1 MODE)")
        while True:
            try:
                t0 = time.time()
    ...
    def _get_price_drift_tolerance(self, symbol: str, result: ForecastResult) -> float:
        """
        Calculate symbol-specific price drift tolerance.
        Base tolerance is 1.5%, with adjustments for volatility.
        """
        base_tolerance = 0.015  # 1.5% base tolerance
        
        # Extremely volatile symbols get higher tolerance
        extreme_volatility_symbols = ["DOGEUSD", "XTZUSD", "XLMUSD", "BCHUSD", "DOTUSD", "SOLUSD"]
        if symbol in extreme_volatility_symbols:
            base_tolerance = 0.025  # 2.5% for extreme volatility
        
        # High volatility symbols get moderate tolerance increase
        high_volatility_symbols = ["ETHUSD", "AVAXUSD", "LINKUSD", "UNIUSD", "AAVEUSD", "MUSD", "NEARUSD", "SUIUSD"]
        if symbol in high_volatility_symbols:
            base_tolerance = 0.020  # 2.0% for high volatility
        
        # Use ATR to adjust tolerance dynamically
        if result and result.atr_candle > 0 and hasattr(self, 'symbol_states') and symbol in self.symbol_states:
            state = self.symbol_states[symbol]
            if state.current_price > 0:
                atr_pct = result.atr_candle / state.current_price
                # If ATR is very high, increase tolerance
                if atr_pct > 0.05:  # ATR > 5% of price
                    base_tolerance = max(base_tolerance, atr_pct * 0.5)  # Use 50% of ATR as extra tolerance
                elif atr_pct > 0.03:  # ATR > 3% of price
                    base_tolerance = max(base_tolerance, atr_pct * 0.4)
                elif atr_pct > 0.02:  # ATR > 2% of price
                    base_tolerance = max(base_tolerance, atr_pct * 0.3)
        
        # Cap tolerance at 5% to prevent extreme slippage
        return min(base_tolerance, 0.05)
    ...
    def _process_symbol(self, symbol: str, state: ApexSymbolState) -> None:
        ...
        state.current_stop_multiplier = 2.2 if state.config.primary_tf == "H1" else result.stop_multiplier
        state.speed_forecasts = result.speed_forecasts
        state.ewmac_forecast = result.ewmac_forecast
        state.breakout_forecast = result.breakout_forecast  # NEW
    ...
    def _open_position(self, symbol: str, state: ApexSymbolState, signal: SignalType, result: ForecastResult) -> None:
        ...
        # ── Fetch a fresh tick immediately before building the stop so the anchor
        #    price reflects the actual fill side (ask for buys, bid for sells),
        #    not the stale mid-price from the last poll cycle.
        fresh_tick = mt5.symbol_info_tick(symbol)
        if fresh_tick is None or fresh_tick.bid == 0:
            logger.warning(f"{symbol}: Cannot get fresh tick for stop calculation – aborting entry")
            return

        fill_reference = fresh_tick.ask if signal == SignalType.BUY else fresh_tick.bid

        # ── Symbol-specific price drift tolerance check ──
        if state.current_price > 0:
            drift_pct = abs(fill_reference - state.current_price) / state.current_price
            
            # Get symbol-specific tolerance
            tolerance = self._get_price_drift_tolerance(symbol, result)
            
            if drift_pct > tolerance:
                logger.warning(
                    f"{symbol}: Price drift {drift_pct*100:.2f}% exceeds tolerance "
                    f"(last={state.current_price:.{digits}f}, now={fill_reference:.{digits}f}, "
                    f"tolerance={tolerance*100:.2f}%) – skipping entry"
                )
                return

        # ── H1 uses 2.2x ATR stops ──
        initial_stop = None
        stop_distance = 0.0
        if self.cfg.atr_trailing.enabled and result.atr_candle > 0:
            if state.config.primary_tf == "H1":
                stop_mult = 2.2
            else:
                stop_mult = result.stop_multiplier

            stop_distance = result.atr_candle * stop_mult
            if signal == SignalType.BUY:
                initial_stop = round(fill_reference - stop_distance, digits)
                if initial_stop >= fill_reference:
                    initial_stop = None
            else:
                initial_stop = round(fill_reference + stop_distance, digits)
                if initial_stop <= fill_reference:
        ...
        logger.info(
            ...
            f"stop_mult={stop_mult if self.cfg.atr_trailing.enabled else 0:.1f}x | "
            f"fill_ref={fill_reference:.{digits}f}"
        )
        ...
        if ticket:
            ...
            if initial_stop:
                ...
                state.position_state.stop_multiplier = stop_mult if self.cfg.atr_trailing.enabled else 2.2
    ...
    def _close_position(self, symbol: str, state: ApexSymbolState, exit_reason: str = "MANUAL", force: bool = False) -> None:
        ...
        if self.order_mgr.close_position(symbol):
            ...
            # Record exit metadata for post-exit re-entry gate
