...
class TradingBot:
    ...
    def __init__(self, config: SystemConfig):
        ...
        self.sizer = VolatilityPositionSizer(
            config.strategy, config.vol_position_sizing, active_count
        )
    ...
    def _validate_and_init_symbols(self) -> None:
        print("\n" + "═" * 80)
        print(f"  SYMBOL VALIDATION REPORT")
        print("═" * 80)
        print(
        ...
        for sym, sym_cfg in self.cfg.symbols.items():
            ...
            self.symbol_states[sym] = SymbolState(
                sym_cfg, self.cfg.strategy, self.cfg.atr_trailing.enabled
            )
    ...
    def _execute_decisions(
        ...
        if state.should_reverse_to_short() and state.can_close_position():
            self._close_position(symbol, state, exit_reason="FORECAST_REVERSAL")
        ...
        if state.should_exit_long() and state.can_close_position():
            self._close_position(symbol, state, exit_reason="FORECAST_FADE")
            return

        if state.should_exit_short() and state.can_close_position():
            self._close_position(symbol, state, exit_reason="FORECAST_FADE")
            return
    ...
    def _open_position(
        ...
        if account.balance < 250:
            logger.warning(
                f"{symbol}: Account balance {account.balance:.2f} ZAR is below R250. "
        ...
        if ticket:
            ...
            logger.info(
                f"{symbol}: OPENED {signal.value} | lots={lots:.4f} | ticket={ticket}"
            )
        else:
