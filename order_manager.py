...
class OrderManager:
    ...
    def update_stop_loss(self, symbol: str, ticket: int, new_stop: float) -> bool:
        ...
        # FIX: Use TRADE_ACTION_MODIFY instead of TRADE_ACTION_SLTP
        # This is more reliable for modifying existing positions
        request = {
        ...
        for attempt in range(1, self.config.max_retries + 1):
            try:
                ...
                if result is None:
                    # Try alternative approach - get fresh position data
                ...
                if result.retcode in (10025, mt5.TRADE_RETCODE_INVALID_STOPS, 10027):
                    logger.warning(
                        f"{symbol}: Invalid stop ({retcode_name}) - stop={rounded_stop}, "
                        f"price={current_price:.{digits}f}, attempt {attempt}"
                    )
                    # Try without TP parameter
                    if "tp" in request:
                        del request["tp"]
                        logger.info(f"{symbol}: Retrying without TP parameter")
                        continue
                    return False
                ...
                logger.error(
                    ...
                    f"retcode={result.retcode} ({retcode_name}), comment='{result.comment}'"
                )
                time.sleep(self.config.retry_delay)
    ...
    def update_take_profit(self, symbol: str, ticket: int, new_tp: float) -> bool:
        """
        Update take profit for an existing position on the broker side.
        
        Returns:
            bool: True if take profit was updated successfully, False otherwise
        """
        # Get current position to preserve stop loss
        positions = mt5.positions_get(ticket=ticket)
        if not positions or len(positions) == 0:
            logger.warning(f"{symbol}: Position ticket {ticket} not found for TP update")
            return False
        ...
        symbol_info = mt5.symbol_info(symbol)
        if not symbol_info:
            logger.error(f"{symbol}: Cannot get symbol info for TP update")
            return False
        ...
        request = {
            ...
            "position": ticket,
            "symbol": symbol,
            "sl": position.sl if position.sl != 0 else 0.0,
            "tp": rounded_tp,
        }
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            logger.info(
                f"✓ TP UPDATE {symbol} | ticket={ticket} | "
                f"new_tp={rounded_tp:.{digits}f}"
            )
            return True
        
        logger.error(
            f"{symbol}: TP update failed - retcode={result.retcode if result else 'None'}"
        )
        return False
                ...
                try:
                    ...
                    if result.retcode == mt5.TRADE_RETCODE_DONE:
                        ...
                        logger.info(
                            ...
                            f"@ {price:.{digits}f} | {stop_str} | ticket={result.order}"
                        )
                        return result.order
