...
from config import EWMACSpeed, StrategyConfig, TIMEFRAME_META, MIN_BARS_FOR_ATR
...
class MultiSpeedEWMAC:
    ...
    def __init__(self, cfg: StrategyConfig, timeframe: str = "M15"):  # Default to M15 for aggressive
        self.cfg       = cfg
        self.timeframe = timeframe
        self.timeframe_meta = TIMEFRAME_META.get(timeframe, TIMEFRAME_META["M15"])
        ...
        logger.info(
            f"MultiSpeedEWMAC (AGGRESSIVE) initialized | timeframe={timeframe} | "
            f"ATR daily multiplier={self.atr_to_daily_multiplier:.2f}x (√{candles_per_day}) | "
            f"Stop multiplier={self.stop_multiplier}x | "
    ...
    def calculate(self, df: pd.DataFrame) -> ForecastResult:
        ...
        if len(df) < MIN_BARS_FOR_ATR:
            logger.warning(f"Only {len(df)} bars available, need at least {MIN_BARS_FOR_ATR}")
            return _empty
        ...
        raw_combined = sum(
            speed.weight * speed_forecasts[speed.name] for speed in self.cfg.speeds
        )
        combined = float(
            np.clip(raw_combined * self.cfg.fdm, self.cfg.cap_min, self.cfg.cap_max)
        )
        ...
        atr_annualized = atr_daily * math.sqrt(252)
        atr_annualized = max(0.01, min(3.5, atr_annualized))  # Increased max to 3.5

        logger.debug(
            f"ATR | tf={self.timeframe} | candle={atr_candle:.6f} | "
            f"daily={atr_daily:.6f} | annual={atr_annualized:.3f} "
            f"({atr_annualized*100:.1f}%) | stop_mult={self.stop_multiplier}x"
        )
    ...
    def _add_atr(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        atr = tr.ewm(
            span=self.cfg.atr_period, adjust=False, min_periods=self.cfg.atr_period
        ).mean()

        if len(atr) < self.cfg.atr_period:
            atr.iloc[: self.cfg.atr_period] = (
                tr.iloc[: self.cfg.atr_period].expanding().mean()
            )
    ...
    def _extract_atr(self, df: pd.DataFrame) -> float:
        ...
        close = df["close"].values[-16:-1]

        if len(high) >= 10:  # Reduced from 14 for faster response
            trs = []
        ...
        return max(current_price * 0.002, current_price * 0.005)
    ...
    def _to_trend(forecast: float) -> str:
        # AGGRESSIVE: Lower threshold for trend detection
        if forecast > 0.5:   # Reduced from 1.0
            return "BULLISH"
        if forecast < -0.5:  # Reduced from -1.0
            return "BEARISH"
        return "NEUTRAL"