...
# ═══════════════════════════════════════════════════════════════════════════
# AGGRESSIVE SIZING PARAMETERS FOR R250 ACCOUNT
# ═══════════════════════════════════════════════════════════════════════════

MAX_MARGIN_FRACTION   = 0.25   # Use up to 25% of free margin per trade
MAX_NOTIONAL_FRACTION = 0.50   # Up to 50% of account per position
MIN_VOLUME_FRACTION   = 0.005  # Minimum 0.5% of capital

MAX_ANNUAL_VOL = 3.5           # Increased from 2.5 - allow higher volatility
MIN_ANNUAL_VOL = 0.50          # Increased from 0.30
...
class VolatilityPositionSizer:
    ...
    def __init__(self, strat_cfg: StrategyConfig, vol_sizing_cfg, active_symbols_count: int, symbol_states: dict = None):
        ...
        self.symbol_states    = symbol_states or {}  # For accessing stop history
        self._usd_zar_rate    = None
        self._last_rate_time  = 0

        logger.info(
            f"VolatilityPositionSizer (AGGRESSIVE) | N={self.expected_positions} | IDM={self.idm:.2f}"
        )
    ...
    def calculate_notional_exposure(
        self, balance_zar: float, annual_vol: float, forecast: float, symbol: str = None
    ) -> float:
        N       = max(self.expected_positions, 1)
        Capital = balance_zar
        T       = self.vol_sizing_cfg.risk_target_pct
        V       = annual_vol if annual_vol > 0 else 0.01
        F       = min(abs(forecast), 30.0)
        IDM     = self.idm

        notional_zar = (1.0 / N) * Capital * (T / V) * (F / 10.0) * IDM

        # Apply dynamic sizing multiplier based on stop loss history
        size_multiplier = 1.0
        if symbol and symbol in self.symbol_states:
            ...
            if size_multiplier < 1.0:
                logger.info(
                    f"{symbol}: Applying size multiplier {size_multiplier:.1f}x "
                    f"(consecutive stops={state.consecutive_stops})"
                )
        ...
        logger.debug(
            f"Notional calc: N={N}, Capital={Capital:.0f}, T={T:.2f}, V={V:.2f}, "
            f"F={F:.1f}, IDM={IDM:.2f}, multiplier={size_multiplier:.2f} → {notional_zar:.2f} ZAR"
        )
        return notional_zar
    ...
    def calculate_lots(
        ...
        if balance_zar < 50:
            logger.warning(f"Account balance {balance_zar:.2f} ZAR below aggressive minimum (R50)")
            return 0.0
