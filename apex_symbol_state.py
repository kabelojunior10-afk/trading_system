...
class ApexSymbolState:
    ...
    def __init__(self, sym_cfg: SymbolConfig, strat_cfg: StrategyConfig, atr_trailing_enabled: bool = True, smart_reentry_cfg=None):
        ...
        stop_mult = 4.0 if sym_cfg.primary_tf == "M1" else 1.5
    ...
    def can_open_position(self) -> bool:
        ...
        cooldown = 15  # Shorter cooldown for M1
    ...
    def can_close_position(self) -> bool:
        if not self.position_state.has_position:
            return False
        if time.time() - self.last_close_attempt < 10:  # Shorter for M1
            return False
        return True
    ...
    def update_trailing_stop(self, current_price: float, current_atr_candle: float = None,
        ...
        # M1 uses 4.0x multiplier for trailing
        if self.config.primary_tf == "M1":
            mult_to_use = 4.0 if stop_multiplier is None else stop_multiplier
        else:
