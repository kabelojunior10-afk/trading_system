...
{Fore.RED}{Style.BRIGHT}!!! ULTRA AGGRESSIVE MODE - M1 TIMEFRAME - HIGHEST RISK LEVEL !!!{Style.RESET_ALL}
...
{Fore.WHITE}Timeframe{Style.RESET_ALL}     : {Fore.RED}M1 (1 minute) - Ultra fast entries{Style.RESET_ALL}
...
{Fore.WHITE}Exit Method{Style.RESET_ALL}    : {Fore.RED}Trailing Stop (4.0× ATR) + Stop and Reverse{Style.RESET_ALL}
...
def print_apex_config_summary(config) -> None:
    print(f"{Fore.RED}{'─' * 80}")
    print(f"APEX SYSTEM CONFIGURATION SUMMARY - M1 TIMEFRAME")
    ...
    print(f"  Primary timeframe       : {Fore.RED}M1 (1 minute){Style.RESET_ALL}")
    ...
    print(f"  • Exit method          : Trailing stop (4.0× ATR) + Signal reversal")

    print(f"{Fore.RED}{'─' * 80}{Style.RESET_ALL}")
    
    print(f"\n{Fore.YELLOW}Press Enter to start APEX M1 trading (Ctrl+C to cancel)...{Style.RESET_ALL}")
    input()
