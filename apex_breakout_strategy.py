...
class MultiSpeedBreakout:
    ...
    def __init__(self, speeds: list, forecast_cap: float = 50.0, forecast_scale: float = 2.0):
        """
        Initialize breakout strategy with multiple speeds.

        Args:
            speeds: List of BreakoutSpeed objects
            forecast_cap: Maximum absolute forecast value
        ...
        logger.info(f"APEX BREAKOUT STRATEGY | Speeds: {[s.name for s in speeds]}")
    ...
    def _calculate_single_breakout(self, df: pd.DataFrame, n: int) -> float:
        ...
        Returns:
            Scaled and capped forecast value
        """
        if len(df) < n:
            return 0.0
        ...
        # Scale to match EWMAC forecast magnitude (approx ±20-50)
        # Breakout signal is ±0.5 at extremes, so multiply by ~40-100
        forecast = scaled_price * self.forecast_scale * 20.0
